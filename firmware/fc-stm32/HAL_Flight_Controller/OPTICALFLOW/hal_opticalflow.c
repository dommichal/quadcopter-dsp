...
enum {
    HAL_OPTICALFLOW_PROCESS_INTERVAL = 100,
};
...
void HAL_OPTICALFLOW_calculate(int16_t velocities[3], float angles[3], float angular_rates[3], HAL_OPTICALFLOW_timepoint_t now) {
    if (OPTICALFLOW_DEVICE.last_process_time + HAL_OPTICALFLOW_PROCESS_INTERVAL < now) {
      if (OPTICALFLOW_DEVICE.state != HAL_OPTICALFLOW_STATE_CONNECTED) {
        ...
        size_t packetSize;
        SerialProtocol_Status status = SerialProtocol_GetPacketRedyToSend(&OPTICALFLOW_DEVICE.serial_protocol_uav.serialProtocol, &OPTICALFLOW_DEVICE.tx_buffer, &packetSize);
        if (status == SERIAL_PROTOCOL_STATUS_OK) {
            HAL_UART_Transmit(&huart1, (uint8_t *)&OPTICALFLOW_DEVICE.tx_buffer, SERIAL_PROTOCOL_MAX_PACKET_LENGTH, HAL_MAX_DELAY);
        }
    }
}
